...
Shader "Mobile/Texture Splat Stencil (Only Directional Lights)" {
	Properties{
	...
	SubShader{
		...
		Pass{
			...
			void surf(Input IN, inout SurfaceOutput o) {
				half4 s = tex2D(_MainTex, IN.uv_splat);

				half4 c1 = tex2D(_Texture1, IN.uv_MainTex) * s.r;
				half4 c2 = tex2D(_Texture2, IN.uv_MainTex) * (1.0 - s.r);

				o.Albedo = c1.rgb * s.r + c2.rgb * (1.0 - s.r);
				o.Alpha = s.a;
			}
